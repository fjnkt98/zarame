...
pub fn main(init: std.process.Init) !void {
    ...
    var opt_lex_file_path: ?[]const u8 = null;

    {
        ...
        while (i < args.len) : (i += 1) {
            ...
            } else if (std.mem.eql(u8, "--lex-file", arg)) {
                i += 1;
                if (i >= args.len) fatal("expected arg after '{s}'", .{arg});
                if (opt_lex_file_path != null) fatal("duplicated {s} argument", .{arg});
                opt_lex_file_path = args[i];
            } else {
    ...
    const lex_file_path = opt_lex_file_path orelse "src/resource/lex.csv";
